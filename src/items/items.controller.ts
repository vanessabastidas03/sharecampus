...
export class ItemsController {
  ...
  findAll(
    ...
    @Query('order_by') order_by?: 'recent' | 'rating',
    @Query('user_campus') user_campus?: string,
  ) {
    return this.itemsService.findAll({ category, campus, offer_type, search, order_by, user_campus });
  }

  @Get('recommended')
  @UseGuards(JwtAuthGuard)
  getRecommended(@Request() req, @Query('campus') campus?: string) {
    return this.itemsService.getRecommended(req.user.sub, campus);
  }
